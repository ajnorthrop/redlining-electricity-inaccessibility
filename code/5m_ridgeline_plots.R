...
holc_311 <- ridges(holc_311_aggregated,"holc_311", outage_rate, holc_grade, 
                   col_holc, "Calls per 1,000 Household-Years", "HOLC Grade")

holc_saifi <- ridges(holc_saifi_aggregated,"holc_saifi", saifi, holc_grade, 
                     col_holc, paste0("SAIFI (Unreliability ","\U2192", ")"), "HOLC Grade")

holc_energy <- ridges(holc_energy_aggregated, "holc_energy", 
                      energy_per_account, holc_grade, col_holc, "Average MJ per Account, Monthly", "HOLC Grade")

ct_311 <- ridges(ct_311_aggregated, "ct_311", outage_rate, ice_quartile, col_ct, "Calls per 1,000 Household-Years", 
                 paste0("ICE Quartile \n(", "Disadvantage ", "\U2192", ")"))
ct_saifi <- ridges(ct_saifi_seasonal, "ct_saifi", saifi, ice_quartile, col_ct, paste0("SAIFI (Unreliability ","\U2192", ")"), 
                   paste0("ICE Quartile \n(", "Disadvantage ", "\U2192", ")"))
ct_energy <- ridges(ct_energy_seasonal, "ct_energy", energy_per_account, ice_quartile, col_ct, "Average MJ per Account, Monthly", 
                    paste0("ICE Quartile \n(", "Disadvantage ", "\U2192", ")"))

both_311 <- ridges(both_311, "both_311", outage_rate, cc, col_both, "Calls per 1,000 Household-Years", "") 
both_saifi <- ridges(both_saifi, "both_saifi", saifi, cc, col_both, paste0("SAIFI (Unreliability","\U2192", ")"), "")
both_energy <- ridges(both_energy, "both_energy", energy_per_account, cc, col_both, "Average MJ per Account, Monthly", "")

holc_311_s <- ridges_seasonal(holc_311_seasonal, "holc_311", outage_rate, holc_grade, col_holc, "Calls per 1,000 Household-Years", "HOLC Grade")
...
holc_energy_s <- ridges_seasonal(holc_energy_seasonal, "holc_energy",energy_per_account, holc_grade, col_holc, "Average MJ per Account, Monthly", "HOLC Grade")

ct_311_s <- ridges_seasonal(ct_311_seasonal, "ct_311", outage_rate, ice_quartile, col_ct, "Calls per 1,000 Household-Years", 
                            paste0("ICE Quartile \n(", "Disadvantage ", "\U2192", ")"))
ct_saifi_s <- ridges_seasonal(ct_saifi_seasonal, "ct_saifi", saifi, ice_quartile, col_ct, paste0("SAIFI (Unreliability ","\U2192", ")"), 
                              paste0("ICE Quartile \n(", "Disadvantaged ", "\U2192", ")"))
ct_energy_s <- ridges_seasonal(ct_energy_seasonal, "ct_energy", energy_per_account, ice_quartile, col_ct, "Average MJ per Account, Monthly", 
                               paste0("ICE Quartile \n(", "Disadvantage ", "\U2192", ")"))

# Step 4: Save aggregate graphs -------------------------------------------------------------------------

holc_311 <- holc_311 + ggtitle("A") + theme(plot.title = element_text(face = "bold", size = 12)) 
holc_saifi <- holc_saifi + ggtitle("B") + theme(plot.title = element_text(face = "bold", size = 12)) 
holc_energy <-  holc_energy + ggtitle("C") + theme(plot.title = element_text(face = "bold", size = 12))
holc_simple <- holc_311 / holc_saifi / holc_energy

ggsave(here("figures","ridge_plots","holc_combined.png"), holc_simple,
              width = 7.47, height = 7, dpi = 300, units = "in", device='png')

ct_311 <- ct_311 + ggtitle("A") + theme(plot.title = element_text(face = "bold", size = 12))
ct_saifi <- ct_saifi + ggtitle("B") + theme(plot.title = element_text(face = "bold", size = 12)) 
ct_energy <- ct_energy + ggtitle("C") + theme(plot.title = element_text(face = "bold", size = 12))
...
holc_311_s <- holc_311_s + ggtitle("A") + theme(plot.title = element_text(face = "bold", size = 16))
holc_saifi_s <- holc_saifi_s + ggtitle("B") + theme(plot.title = element_text(face = "bold", size = 16))
holc_energy_s <- holc_energy_s + ggtitle("C") + theme(plot.title = element_text(face = "bold", size = 16))
...
ct_311_s <- ct_311_s + ggtitle("A") + theme(plot.title = element_text(face = "bold", size = 16)) 
ct_saifi_s <- ct_saifi_s + ggtitle("B") + theme(plot.title = element_text(face = "bold", size = 16))
ct_energy_s <- ct_energy_s + ggtitle("C") + theme(plot.title = element_text(face = "bold", size = 16))
...
both_311 <- both_311 + ggtitle("A") + theme(plot.title = element_text(face = "bold", size = 16),
                                            axis.text.y = element_text(vjust = -2))
both_saifi <- both_saifi + ggtitle("B") + theme(plot.title = element_text(face = "bold", size = 16),
                                                axis.text.y = element_text(vjust = -2))
both_energy <- both_energy + ggtitle("C") + theme(plot.title = element_text(face = "bold", size = 16),
                                                  axis.text.y = element_text(vjust = -2))
