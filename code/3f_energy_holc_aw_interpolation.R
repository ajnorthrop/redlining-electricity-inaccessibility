...
energy_use_holc <- value_all %>% 
  left_join(account_all, by = c("unique_id", "holc_grade", "source")) %>% 
  mutate(energy_per_account = value/number_of_accounts) 

energy_use_holc_cool <- value_cool %>% 
  left_join(account_cool, by = c("unique_id", "holc_grade", "source")) %>% 
  mutate(energy_per_account = value/number_of_accounts) 

energy_use_holc_warm <- value_warm %>% 
  left_join(account_warm, by = c("unique_id", "holc_grade", "source")) %>% 
  mutate(energy_per_account = value/number_of_accounts) 

# Step 5: Save data-------------------------------------------------------------------------

write_csv(energy_use_holc, here("data","energy","holc_energy_use_aggregated.csv"))

write_csv(energy_use_holc_cool, here("data","energy","holc_energy_use_cool.csv"))

write_csv(energy_use_holc_warm, here("data","energy","holc_energy_use_warm.csv"))